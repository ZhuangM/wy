...
import com.up.wyglxt.AppConstants;
import com.up.wyglxt.base.BaseDAO;
import com.up.wyglxt.entity.Charge;

public class ChargeDAO extends BaseDAO {
	private static final Logger log = Logger.getLogger(ChargeDAO.class);
	...
	public Charge loadByHouseNoAndMonth(String houseNo, Date month) {
		...
		Session session = getCurrentSession();
		result = (Charge) session.createCriteria(Charge.class).add(Restrictions.eq("houseNo", houseNo))
				.add(Restrictions.eq("month", month)).uniqueResult();
		return result;
	}
}
